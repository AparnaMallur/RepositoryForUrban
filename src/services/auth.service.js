...
update_session(id) {
    console.log("update session of user called")
    return axios
      .post(API_URL + "api/session_update", { id })
      .then((response) => {
  ...
  otpCheck(email_id, otp) {
    
    //var otp = otp;

    return axios
      .post(API_URL + "api/check_otp", {email_id, otp})
      .then((response) => {
  ...
  userRegisterPageTwo(register_user_id, cat_id) {
    ...
    console.log("service user two regsiter cat_id : ");
    console.log(cat_id);
    console.log("service user two regsiter typeof cat_id : ");
    ...
    return axios
      ...
      .then((response) => {
        ...
        console.log("auth serivce response:");
        console.log(response);
  ...
  userRegisterPageThree(register_user_id, ngo_id) {
    ...
    console.log("service user two regsiter ngo_id : ");
    console.log(ngo_id);
    console.log("service user two regsiter typeof ngo_id : ");
    console.log(typeof ngo_id);
  ...
  NgoNextRegProcedureFour(id, skip, next, cat_id, celebrity_name, celebrity_desc, celebrity_image, year_var, desc_var, username, about_us, image, cover_image, ceo_speech, video) {
    ...
    if(year_var && typeof year_var !=="undefined" && year_var.length>0){
      for (var i = 0; i < year_var.length; i++) {  
        insertData.append('year[]', year_var[i]);
      }
    }else{
    ...
    if(desc_var && typeof desc_var !=="undefined" && desc_var.length>0){
      for (var i = 0; i < desc_var.length; i++) {  
        insertData.append('desc[]', desc_var[i]);
      }
    }else{
  ...
  forgotPasswordOtpVrf(email_id, otp) {
    return axios
      .post(API_URL + "api/forgot_password_otp_verification", {email_id, otp})
      .then((response) => {
  ...
  forgotPasswordSave(new_password, confirm_password, email_id) {
    return axios
      .post(API_URL + "api/save_new_password", {new_password, confirm_password, email_id})
      .then((response) => {
